...
from clients.claude import stream_action, stream_recovery_action, stream_plan
...
async def plan_task(state: AgentState) -> AgentState:
    """Decompose the task into a checklist once, before the first action."""
    logger.info("[PLAN] task=%r  url=%s  dom_nodes=%d",
                state.task, state.context.url, len(state.context.dom_tree))
    try:
        checklist = await stream_plan(state)
    except Exception as exc:
        logger.warning("[PLAN] failed (%s), using single-step fallback", exc)
        checklist = ""
    state.checklist = checklist or f"[ ] {state.task}"
    logger.info("[PLAN] checklist:\n%s", state.checklist)
    return state
...
async def verify(state: AgentState) -> AgentState:
    ...
    if len(state.actions_taken) >= 2:
        ...
        if last.type in (ActionType.CLICK, ActionType.TYPE, ActionType.NAVIGATE,
                         ...
                         ActionType.RELOAD, ActionType.NEW_TAB):
            current_hash = hash_dom(state.context.dom_tree)
            if (state.previous_dom_hash == current_hash and
                state.previous_url == state.context.url):
                is_stuck = True
